...
# Отображение версии операционной системы и другой информации
echo "=== Информация об ОС ==="
grep -E '^PRETTY_NAME' /etc/os-release | awk -F '="' '{print $2}' | sed 's/"$//'

# Время безотказной работы системы
echo "=== Время безотказной работы системы ==="
uptime -p

# Вошедшие в систему пользователи
echo "=== Вошедшие в систему пользователи ==="
who
